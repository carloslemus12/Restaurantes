        ...
        <div class="card text-center">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('restaurant.statistic') }}">Restaurantes</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="{{ route('saucers.statistic') }}">Platillos</a>
                    </li>
                </ul>
            </div>
            <div class="card-body">
                        ...
                        @foreach ($platillos as $platillo)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <script> 
                                        comentarios['{{ $platillo->id }}'] = @json($platillo->comentarios);
                                        
                                        @foreach ($platillo->comentarios as $comentario)
                                            comentarios_usuario['{{ $comentario->id }}'] = @json($comentario->usuario);                                       
                                        @endforeach

                                        recomendaciones['{{ $platillo->id }}'] = @json($platillo->recomendaciones);
                                        
                                        @foreach ($platillo->recomendaciones as $recomendaciones)
                                ...
                                <td>
                                    <button id="btn_chat" restaurante="{{ $platillo->id }}" type="button" class="btn btn-primary bg-transparent" data-toggle="modal" data-target="#modal_chat">
                                        <img src="{{ asset('img/chat.png') }}" />
                                    </button>
                                    <button id="btn_box" restaurante="{{ $platillo->id }}" type="button" class="btn btn-dark bg-transparent" data-toggle="modal" data-target="#modal_box">
